...
gamF=max(min(0.5*ones(nf,1)+rr*randn(nf,1),0.99),0.01);
gamM=max(min(0.1*ones(nm,1)+rr*randn(nm,1),0.99),0.01);
gam=[gamF;gamM];
...
lamF=max(min(.1*ones(nf,1)+rr*randn(nf,1),0.99),0.01);
lamM=max(min(.5*ones(nm,1)+rr*randn(nm,1),0.99),0.01);
lam=[lamF;lamM];
...
% figure;plot(0:tf-1,phi,'linewidth',1.3)
% figure;plot(0:tf-1,phiN,'linewidth',1.3)
% figure;plot(0:tf-1,p,'linewidth',1.3)
% hold on
% plot(0:tf,max(b)*ones(size(0:tf,2)))
% figure;plot((phiD))
% figure;plot(0:tf-1,dd-p,'linewidth',1.3)
% figure;plot(T,'o','markersize',5)
% figure;plot(TT,'o','markersize',5)
